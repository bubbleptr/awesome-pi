...
所有徽章左侧统一使用透明底的绿色 π，沿用 `favicon.svg` 的字形，去掉灰色底栏和图标方块；内嵌 SVG 为 20×20，左侧预留宽 24，整体高 20。右侧使用原徽章的黑灰色（#555）圆角底和白色 Website / 插件名文字，背景和渐变仅绘制在标签区域。SVG 的 `title` / `aria-label` 继续包含 Pi Index 品牌名。详情页声明的图片宽度须与生成的 SVG 一致，避免图片被拉伸。
