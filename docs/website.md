...
所有徽章按用户明确纠正的左右配色：左侧黑灰底（#555）＋绿色 π（#187a45），右侧绿底（#187a45）＋白色 Website / 插件名文字。π 沿用 favicon 字形，但不带独立方块；内嵌 SVG 为 20×20，左栏宽 24，整体高 20，外轮廓保持圆角。SVG 的 `title` / `aria-label` 继续包含 Pi Index 品牌名，详情页声明尺寸须与图片自然尺寸一致。不要将右侧改为灰色，也不要把左侧底栏误删为透明。
